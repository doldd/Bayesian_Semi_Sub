...
def main(batch_size, reuse_artifact, split, max_epochs, weight_decay, lr, seed, num_bends, num_samples, num_warmup, num_chains, gpu_dev):
    ...
    # # load subspace model
    # mean, cov = get_curve_space_torch(best_model)
    # device = trainer._accelerator_connector.accelerator.root_device
    # subspace_model = SemiSubspaceModel(mean, cov, SimpleCnn(), 0).to(device=device)

    # metric_collection_base = MetricCollection([
    #     CalibrationError(),
    #     AUROC(),
    # ]).to(device=device)
    # # do grid
    # x = np.linspace(-20, 20, 10, dtype=np.float32)
    # y = np.linspace(-6, 8, 10, dtype=np.float32)
    # xx, yy = np.meshgrid(x, y)
    # grid = np.vstack([xx.flatten(), yy.flatten()]).T
    # all_metrics = []
    # metric_collection = metric_collection_base.clone()

    # cuda_loader = cudaIter(device=device, bs=422, data_set=dm.ds_train)
    # with torch.no_grad():
    #     for p in tqdm(grid):
    ...
    #         nll = 0.
    #         for data, y in cuda_loader:
    #             y = y.cuda() if y.device is not device else y
    #             p_pred = subspace_model(data).squeeze()
    #             nll += torch.nn.BCEWithLogitsLoss(reduction='sum')(p_pred, y).item()
    #             metric_collection.update(torch.sigmoid(p_pred), y.to(dtype=torch.int32))

    #         metrics = metric_collection.compute()
    #         metrics['nll'] = nll / len(dm.ds_train)
    #         all_metrics.append(metrics)
    # df = pd.DataFrame(all_metrics)
    # df['CalibrationError'] = df['CalibrationError'].apply(lambda x: x.item())
    # df['AUROC'] = df['AUROC'].apply(lambda x: x.item())
    # df['xx'] = xx.flatten()
    # df['yy'] = yy.flatten()
    ...
    # t0 = p_inv @ (w0.cpu().numpy() - mean.cpu().numpy())
    # t12 = p_inv @ (w12.cpu().numpy() - mean.cpu().numpy())
    # t2 = p_inv @ (w2.cpu().numpy() - mean.cpu().numpy())
    # fig = plot_subspace(df, "nll", t0, t12, t2, linear_color=False, interpolate=True)
    # wandb_logger.experiment.log({"train_grid_nll": wandb.Image(fig)})
    # fig = plot_subspace(df, "AUROC", t0, t12, t2, linear_color=False, interpolate=True)
    # wandb_logger.experiment.log({"train_grid_AUROC": wandb.Image(fig)})
    # w_table = wandb.Table(dataframe=df)
    ...
    #     sequential_dnn=model_structure.dnn,
    #     module_structure=model_structure.structure_lin,
    #     outcome_dist=outcome_dist)

    # pyro.set_rng_seed(seed+3)
    # loader = DataLoader(dm.ds_train,
    #                     batch_size=len(dm.ds_train),
    #                     shuffle=False,
    #                     num_workers=0,
    #                     pin_memory=False)
    # for (u_, x_), y_ in loader:
    #     u_train = u_.to(device=device)
    #     x_train = x_.to(device=device)
    #     y_train = y_.to(device=device)
    # print(u_.device)
    ...
    #                                         'num_samples': num_samples,
    #                                         'seed': seed+3,
    #                                         'Subspace_dimension': num_bends - 1})
    # # create arviz inference object
    ...
    #     loader = dm.val_dataloader(len(dm.ds_val))
    #     (u_valid, x_valid), y_valid = list(loader)[0]
    ...
    #     loader = dm.test_dataloader(len(dm.ds_test))
    #     u_test, x_test, y_test = list(loader)[0]
    ...
    #         dataset = list([u_train, x_train, y_train]),
